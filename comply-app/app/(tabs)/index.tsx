...
// ── PENALTY CALCULATOR ────────────────────────────────────────────────────────
// Returns estimated penalty in euros/local currency based on category + revenue
function calcPenalty(category: string, monthlyRevenue: number, dueDate: string, done: boolean): {
  ...
  // Penalty formulas by category
  const formulas: Record<string, { fixed: number; rate: number; currency: string }> = {
    tax:       { fixed: 200,  rate: 0.10, currency: '€' }, // fixed fine + 10% of quarterly tax
    license:   { fixed: 250,  rate: 0.00, currency: '€' }, // flat fine
    payroll:   { fixed: 500,  rate: 0.05, currency: '€' }, // fixed + 5% of monthly payroll
    insurance: { fixed: 0,    rate: 0.00, currency: '€' }, // loss of coverage
    filing:    { fixed: 150,  rate: 0.02, currency: '€' }, // fixed + 2%/month
  };

  const f            = formulas[category] || formulas.tax;
  const basePenalty  = f.fixed + (quarterly * f.rate);
  const dailyRate    = basePenalty / 30;
  const totalPenalty = isOverdue
    ? basePenalty + (dailyRate * daysLate)
    : basePenalty;
...
export default function HomeScreen() {
  ...
  // Live penalty counter — ticks every second for overdue deadlines
  ...
  // Start/stop live penalty counter when a deadline is selected
  useEffect(() => {
    if (selected && selected.due_date && !selected.done) {
      ...
      if (calc.isOverdue) {
        const dailyRate = calc.totalPenalty / Math.max(calc.daysLate, 1);
        const secondRate = dailyRate / 86400;
        penaltyTimer.current = setInterval(() => {
          setLivePenalty(prev => Math.round((prev + secondRate) * 100) / 100);
        }, 1000);
      }
    } else {
      setLivePenalty(0);
      ...
      <View style={styles.flex}>
        ...
        {totalAtRisk > 0 && (
          <View style={styles.riskBanner}>
            ...
            <Text style={styles.riskAmount}>{formatMoney(totalAtRisk, '€')}</Text>
          </View>
        )}
        ...
        {/* ── DEADLINE DETAIL MODAL WITH PENALTY CALCULATOR ── */}
                      ...
                      <View style={calc.isOverdue ? styles.penaltyBoxDanger : styles.penaltyBox}>
                        {calc.isOverdue ? (
                          <>
                            ...
                            <Text style={styles.penaltyCounterDanger}>{formatMoney(livePenalty, '€')}</Text>
                            <Text style={styles.penaltySubDanger}>
                              {calc.daysLate} day{calc.daysLate!==1?'s':''} late · Growing every second
                            </Text>
                          </>
                        ) : (
                          <>
                            <Text style={styles.penaltyLabel}>⚠️ Estimated penalty if missed</Text>
                            <Text style={styles.penaltyCounter}>{formatMoney(calc.totalPenalty, '€')}</Text>
                            <Text style={styles.penaltySub}>
                              Based on your €{monthlyRevenue.toLocaleString()}/month revenue
                            </Text>
                          </>
                        )}
                      </View>
...
function DeadlineRow({ d, monthlyRevenue, onToggle, onPress }: {
  d: Deadline; monthlyRevenue: number;
  onToggle:(id:number)=>void; onPress:()=>void
}) {
